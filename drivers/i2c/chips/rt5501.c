...
static int rt5501_headset_detect(int on)
{
	...
	cpld_gpio_write(CPLD_EXT_GPIO_MIC_2V85_EN, on);
#endif
...
#if defined(CONFIG_CPLD) && (defined(CONFIG_MACH_CP3DUG) || defined(CONFIG_MACH_DUMMY) || defined(CONFIG_MACH_DUMMY) || defined(CONFIG_MACH_DUMMY) || defined(CONFIG_MACH_DUMMY))
	cpld_gpio_write(pdata->gpio_rt5501_spk_en, 0);
#else
...
#if defined(CONFIG_CPLD) && (defined(CONFIG_MACH_CP3DUG) || defined(CONFIG_MACH_DUMMY) || defined(CONFIG_MACH_DUMMY) || defined(CONFIG_MACH_DUMMY) || defined(CONFIG_MACH_DUMMY))
		cpld_gpio_write(pdata->gpio_rt5501_spk_en, 1);
#else
...
err2:
	...
	case RT5501_SET_AMP:
		if (copy_from_user(&rt5501_onoff, argp, sizeof(rt5501_onoff)))
			return -EFAULT;
		rc = 0;
		if (rt5501_onoff == 1)
			set_rt5501_amp(1);
		else if (rt5501_onoff == 0)
			set_rt5501_amp(0);
		else {
...
int rt5501_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if(pdata->gpio_rt5501_spk_en) {
		...
		for(retrycnt = 0;retrycnt < 10; retrycnt++ ) {
			...
			ret2 = cpld_gpio_write(pdata->gpio_rt5501_spk_en, 1);
			if(ret2 < 0)
				pr_err("%s: write error AUD_HP_EN\n", __func__);

			usleep(5000); 
			ret = rt5501_i2c_read(temp, 2);
			if(ret < 0) {
				pr_err("rt5501 is not connected, retry %d\n",retrycnt);
				ret2 = cpld_gpio_write(pdata->gpio_rt5501_spk_en, 0);
				if(ret2 < 0)
					pr_err("%s: write error AUD_HP_EN to 0\n", __func__);
				usleep(5000); 
			} else
				break;
		}
...
#endif
		if(ret < 0) {
			pr_err("rt5501 is not connected\n");
			rt5501Connect = 0;
		} else {
		...
		ret = cpld_gpio_write(pdata->gpio_rt5501_spk_en, 0);
		if(ret < 0)
			pr_err("%s: write error AUD_HP_EN\n", __func__);
#else
		gpio_direction_output(pdata->gpio_rt5501_spk_en, 0);
