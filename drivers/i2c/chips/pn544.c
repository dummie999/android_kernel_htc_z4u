				...
				const char *buf, size_t count)
{
	...
	case 1:
			...
			break;
	case 2:
			I("%s: case 2, pn544_Disable\n", __func__);
			pn544_Disable();
			break;
	case 3:
			I("%s: case 3, pn544_Enable\n", __func__);
			pn544_Enable();
			break;
	case 4:
			I("%s: case 4\n", __func__);
	...
	I("%s: irq = %d,  ven_gpio = %d,  firm_gpio = %d -\n", __func__, \
		gpio_get_value(pni->irq_gpio), pn544_isEn(), pni->get_firm_gpio());
	return count;
}
		...
		const struct i2c_device_id *id)
{
	...
	if (platform_data->set_ven_gpio != NULL) {
		I("%s: set_ven_gpio\n", __func__);
		pni->set_ven_gpio = platform_data->set_ven_gpio;
	}
	if (platform_data->get_ven_gpio != NULL) {
		I("%s: get_ven_gpio\n", __func__);
		pni->get_ven_gpio = platform_data->get_ven_gpio;
	}
	if (platform_data->set_firm_gpio != NULL) {
		I("%s: set_ven_gpio\n", __func__);
		pni->set_firm_gpio = platform_data->set_firm_gpio;
	}
	if (platform_data->set_firm_gpio != NULL) {
		I("%s: set_firm_gpio\n", __func__);
		pni->get_firm_gpio = platform_data->get_firm_gpio;
	}
