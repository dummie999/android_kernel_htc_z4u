...
static DEFINE_MUTEX(port_mutex);
	...
	struct uart_port *sprd_uart_port;
#endif
...
int sprd_uart_write(void)
{
	...
	if (!sprd_uart_port) {
		printk(KERN_ERR "sprd_uart_port not exist\n");
		return -ENODEV;
	} else {
		port = sprd_uart_port;
	}
	...
	if (!circ->buf) {
		printk(KERN_ERR "sprd_uart buf not exist please check whether it is opened\n");
		return -ENOENT;
	}

	if (count > UART_XMIT_SIZE) {
		printk(KERN_ERR "sprd_uart_port buf too large\n");
		return -EINVAL;
	}
	...
	if (c == 0) {
		memset(circ->buf,sprd_buf,count);
	} else if(count > c) {
		memset(circ->buf + circ->head,sprd_buf,c);
		memset(circ->buf,sprd_buf,count - c);
	} else {
		memset(circ->buf + circ->head,sprd_buf,count);
	}

	circ->head = (circ->head + count) & (UART_XMIT_SIZE - 1);

	if (c <= 0) {
		printk(KERN_ERR "sprd uart has no space\n");
		spin_unlock_irqrestore(&port->lock, flags);
		return -ENOENT;
	}
...
#ifdef CONFIG_SPRD_FLASHLESS
	if ((uport->line == 1) && (!strncmp("ttyHS",dev_name(tty_dev),5))) {
		printk(KERN_INFO "SPRD serial_hs port init\n");
		sprd_uart_port = uport;
	}
#endif
