...
static int cable_detect_get_type(struct cable_detect_info *pInfo)
{
	...
	if (pInfo->usb_id_pin_gpio == ID_PIN_CPLD) {
		...
		if(value < 0)  printk(KERN_ERR "[CABLE]read error CPLD_EXT_GPIO_USB_ID_INPUT_LEVEL");
	} else {
...
void cpid_usbid_interrupt(unsigned long data)
{
	struct cable_detect_info *pInfo = (struct cable_detect_info *)data;

	if (!atomic_read(&cpld_irqen))
		return;
	atomic_set(&cpld_irqen,0);
...
static void mhl_status_notifier_func(bool isMHL, int charging_type)
{
	...
	int id_pin;
	if (pInfo->usb_id_pin_gpio == ID_PIN_CPLD) {
		id_pin = cpld_gpio_read(CPLD_EXT_GPIO_USB_ID_INPUT_LEVEL);
		if(id_pin < 0)  printk(KERN_ERR "[CABLE]read error CPLD_EXT_GPIO_USB_ID_INPUT_LEVEL");
	} else {
				...
				const char *buf, size_t count)
{
	...
	if (!!wakeup) {
		if (pInfo->usb_id_pin_gpio == ID_PIN_CPLD) {
			...
			cpld_gpio_write(CPLD_EXT_GPIO_EXT_USB_ID_OUTPUT, 0);
			msleep(1);
...
void check_usb_id(struct cable_detect_info *pInfo)
{
	if (!cpld_gpio_read(CPLD_EXT_GPIO_USB_ID_INPUT_LEVEL)) {
		CABLE_INFO("CPLD_USB_ID low level before booting\n");
		cpid_usbid_interrupt((unsigned long)pInfo);
	}
}
