...
#define SPRD_SC800x_GPIO_MDM_ALIVE 29
#endif
...
static int ts27010_close_channel(u8 dlci)
{
	...
	mux_print(MSG_CRIT, "closing dlci %d\n", dlci);
...
EXIT:
	...
	mux_print(MSG_CRIT, "DLCI %d is closed, ret = %d\n", dlci, retval);
...
void ts27010_mux_uart_line_close(int line)
{
	...
	mux_print(MSG_CRIT, "prepared to close line = %d\n", line);
...
int ts27010_mux_uart_line_ioctl(unsigned int cmd, unsigned long arg, int line)
{
	...
	case TS0710MUX_IO_SET_DEBUG_LEVEL:
		if ((arg >= MSG_MSGDUMP) && (arg <= MSG_NONE)) {
			...
			pr_err("MUX_UART: ignore invalid debug level = %lu\n",
				arg);
		}
	...
	for (i = 0; i < count; i++) {
		...
		case RECV_STATE_ADDR:
			if (!ts0710_valid_dlci(c >> 2)) {
				mux_print(MSG_ERROR,
					"RECV_STATE_ADDR RX wrong data %02x, Drop msg.\n", c);
		...
		case RECV_STATE_CONTROL:
			if (!ts0710_valid_control(c)) {
				mux_print(MSG_ERROR,
					"RECV_STATE_CONTROL RX wrong data %02x, Drop msg.\n", c);
				ts27010_mux_dump_ringbuf(rbuf, start_flag);
				consume_idx = start_flag;
				i = start_flag;
				state = RECV_STATE_IDLE;
