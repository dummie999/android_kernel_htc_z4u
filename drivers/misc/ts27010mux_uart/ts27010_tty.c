...
#ifdef UART_MUX_LOG
	struct ts27010_ringbuf		*logbuf;
	struct workqueue_struct 	*mux_log_wq;
#endif
};

#ifdef UART_MUX_LOG
#define RECV_RUNNING 0
DEFINE_MUTEX(mux_log_lock);
long log_count;
int log_m;
struct ts27010_tty_data *tdlog;
struct tty_struct *mux_log_tty;
static unsigned long s_muxlog_recv_flags;
void mux_log_recv_worker(struct work_struct *work);
static DECLARE_DELAYED_WORK(mux_log_queue, mux_log_recv_worker);
#endif
...
#ifdef UART_MUX_LOG
static void mux_log_count(int len)
{
	FUNC_ENTER();
	log_count += len;

	if ((log_count > 0) && ((log_count >> 20) > log_m)) {
		mux_print(MSG_ERROR,
			"Log has lost %ld MB\n",
				log_count/1024/1024);
		log_m = log_count >> 20;
...
void mux_log_recv_worker(struct work_struct *work)
{
	...
	FUNC_ENTER();
	if (atomic_read(&tdlog->chan[UART_LOG_CHANNEL].ref_count) <= 0) {
		mux_print(MSG_ERROR, "Mux log channel already closed\n");
		return;
	}

	if (test_and_set_bit(RECV_RUNNING, &s_muxlog_recv_flags)) {
		mux_print(MSG_WARNING, "another recv_work is running\n");
		return;
	}

	if (!mux_log_tty) {
		mux_print(MSG_ERROR, "mux log device not exists\n");
		return;
	}
	mutex_lock(&mux_log_lock);
	if (atomic_read(&tdlog->chan[UART_LOG_CHANNEL].ref_count) <= 0) {
		mux_print(MSG_ERROR, "Mux log channel already closed\n");
		mutex_unlock(&mux_log_lock);
		goto EXIT;
	}

	left = ts27010_ringbuf_room(tdlog->logbuf);

	if (left == LOG_BUFFER_SIZE - 1) {
		mux_print(MSG_WARNING, "Don't queue empty data\n");
		mutex_unlock(&mux_log_lock);
		goto EXIT;
	}
	data_len = LOG_BUFFER_SIZE - left - 1;

	for(;;) {
		avail = data_len <= 2048?data_len:2048;
		if (!mux_log_tty) {
			WARN_ON(mux_log_tty==NULL);
			break;
		}
		avail = tty_prepare_flip_string(mux_log_tty, &buf, avail);
		if (avail <= 0) {
			break;
		}
		count = ts27010_mux_log_ringbuf_read(tdlog->logbuf, 0, buf, avail);
		if (count != avail)
			mux_print(MSG_ERROR, "read size %d is not %d\n", count, avail);
		tty_flip_buffer_push(mux_log_tty);
	...
	if (ts27010_ringbuf_room(tdlog->logbuf) > MUX_LOG_WATERMARK_HIGH)
		mux_log_throttle(0);

	if (data_len) {
		queue_delayed_work(tdlog->mux_log_wq, &mux_log_queue,
...
EXIT:
	clear_bit(RECV_RUNNING, &s_muxlog_recv_flags);
	FUNC_EXIT();
	return;
}
#endif
...
#ifdef UART_MUX_LOG
	if (line == UART_LOG_CHANNEL) {
		if (ts27010_ringbuf_room(tdlog->logbuf) <= MUX_LOG_WATERMARK_LOW)
			mux_log_throttle(1);
		mutex_lock(&mux_log_lock);
		ret = ts27010_mux_log_write(tdlog->logbuf, buf, len);
		mutex_unlock(&mux_log_lock);
		if (ret)
			mux_log_count(ret);
	} else
...
static void ts27010_tty_close(struct tty_struct *tty, struct file *filp)
{
	...
	mux_print(MSG_CRIT, "tty device /dev/mux%d will be closed by %s:%d\n",
		line, current->comm, current->pid);
#ifdef HAS_PS_IND
	if (line == TS0710_MAX_MUX) {
		ts27010_uart_tty_table[line] = NULL;
...
#endif
		mux_print(MSG_CRIT, "tty /dev/mux%d closed successfully\n",
			line);
	}
	FUNC_EXIT();
}
...
#ifdef UART_MUX_LOG
	td->logbuf = ts27010_ringbuf_alloc(LOG_BUFFER_SIZE);
	if (td->logbuf == NULL) {
		err = ENOMEM;
		goto err2;
	}
...
#ifdef UART_MUX_LOG
	tdlog = td;
	td->mux_log_wq = create_workqueue("sprd_mux_log");
#endif
...
void ts27010_tty_uart_remove(void)
{
	...
	FUNC_ENTER();
#ifdef UART_MUX_LOG
	destroy_workqueue(td->mux_log_wq);
	kfree(td->logbuf);
	td->logbuf = NULL;
	tdlog = NULL;
#endif
