...
static int msm_csic_config(struct csic_cfg_params *cfg_params)
{
	...
	pr_info("%s MIPI_PHY_CL_CONTROL val=0x%x\n", __func__, val);
	...
	pr_info("%s MIPI_PHY_D1_CONTROL val=0x%x\n", __func__, val);
...
static irqreturn_t msm_csic_irq(int irq_num, void *data)
{
    ...
    static u32 irq_last = 0, ptl_last = 0, same_cnt = 0;
	irq = msm_io_r(csic_dev->base + MIPI_INTERRUPT_STATUS);
	ptl = msm_io_r(csic_dev->base + MIPI_PROTOCOL_CONTROL);
	if(Print_csic_irq_cnt <= 0) {
	}
