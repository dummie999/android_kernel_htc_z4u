...
void Yushan_Debug_Reg_Dump(uint8_t ctrl)
{
	uint8_t	udwSpiData = 0;


	switch(ctrl) {
		case REG_DUMP_TX_FRAME_COUNT:	
			SPI_Read(YUSHAN_CSI2_TX_FRAME_NO_0, 4, (uint8_t *)(&udwSpiData));
			pr_info("%s TX frame count = %d", __func__, udwSpiData);
			break;

		case REG_DUMP_RX_FRAME_COUNT:	
			SPI_Read(YUSHAN_CSI2_RX_FRAME_NUMBER, 4, (uint8_t *)(&udwSpiData));
			pr_info("%s RX frame count = %d", __func__, udwSpiData);
			break;
