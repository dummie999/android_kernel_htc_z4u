...
static void msm72k_shutdown(struct platform_device *pdev)
{
	...
	if (ui == NULL || ui->pdev == NULL) {
		pr_err("%s: resource released\n\n", __func__);
		return;
	}

	dev_info(&ui->pdev->dev, "%s\n", __func__);

	if (is_usb_online(ui)) {
		
		dev_info(&ui->pdev->dev, "%s: pull down D+\n", __func__);
		msm72k_pullup_internal(&ui->gadget, 0);
	}
}
