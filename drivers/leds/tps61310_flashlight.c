...
#endif
	FLT_INFO_LOG("%s\n", __func__);
	if (this_tps61310->cpld) {
#if defined(CONFIG_CPLD)
		ret = cpld_gpio_write(CPLD_EXT_GPIO_FLASH_EN, 0);
		if (ret < 0)
			printk(KERN_ERR "write error CPLD_EXT_GPIO_FLASH_EN");
		ret = cpld_gpio_write(CPLD_EXT_GPIO_TORCH_FLASH, 1);
		if (ret < 0)
			printk(KERN_ERR "write error CPLD_EXT_GPIO_TORCH_FLASH");
#endif
	} else {
		...
		if (this_tps61310->enable_FLT_1500mA) {
			...
			switch (mode) {
				...
				case FL_MODE_FLASH:
					...
					if (this_tps61310->cpld) {
#if defined(CONFIG_CPLD)
						ret = cpld_gpio_write(CPLD_EXT_GPIO_TORCH_FLASH, 0);
						if (ret < 0)
							printk(KERN_ERR "write error CPLD_EXT_GPIO_TORCH_FLASH");
						ret = cpld_gpio_write(CPLD_EXT_GPIO_FLASH_EN, 1);
						if (ret < 0)
							printk(KERN_ERR "write error CPLD_EXT_GPIO_FLASH_EN");
#endif
					} else {
