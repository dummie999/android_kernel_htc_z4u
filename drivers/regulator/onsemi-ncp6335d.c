...
#define NCP_FOOTPRINT(offset, value) do { *(uint32_t *)(virt_start_ptr + offset) = value;} while(0);
#define NCP_DUMP_VAL	0x100
#define NCP_NEW_UV	0x104
#define NCP_SET_UV	0x108
#define NCP_VAL_UV	0x10C
			...
			unsigned int reg, const char *func)
{
	...
	NCP_FOOTPRINT(NCP_DUMP_VAL, val);
			...
			int min_uV, int max_uV, unsigned *selector)
{
	...
	if (new_uV > (max_uV + NCP6335D_STEP_VOLTAGE_UV)) {
		dev_err(dd->dev, "Unable to set volatge: %d (%d %d)\n",
							new_uV, min_uV, max_uV);
		rc = -EINVAL;
		goto err_set_vtg;
	}

	if(new_uV < CP3_MIN_UV){
		dev_err(dd->dev, "Unable to set volatge: %d (%d %d)\n",
							new_uV, min_uV, max_uV);
		rc = -EINVAL;
		goto err_set_vtg;
...
__retry__:
	...
	if (rc) {
		...
		NCP_FOOTPRINT(NCP_NEW_UV, new_uV);
		NCP_FOOTPRINT(NCP_SET_UV, set_val);
		NCP_FOOTPRINT(NCP_VAL_UV, temp);

		dev_dbg( dd->dev, "NCP: %s new_uV:%d set_val:0x%x Val:0x%x\n", __func__, new_uV, set_val, temp);
		if (ncp6335d_get_voltage(rdev) != new_uV) {
			dev_err( dd->dev, "NCP: ERR %s set new uV (%d) was not applied corrently (current uV: %d) sel_val:0x%x Val:0x%x!!!\n", __func__, new_uV, dd->curr_voltage, set_val, temp);
			dump_registers(dd, dd->vsel_reg, __func__);
			retry++;
			if (retry < 3) {
				goto __retry__;
...
#ifdef CONFIG_HTC_ACPU_DEBUG
		msm_proc_comm( PCOM_BACKUP_NCP_VOLTAGE, (unsigned*)&new_uV, 0);
#endif
	}
