...
static struct worker *create_worker(struct global_cwq *gcwq, bool bind)
{
	...
	while (ida_get_new(&gcwq->worker_ida, &id)) {
		...
		if (!ida_pre_get(&gcwq->worker_ida, GFP_KERNEL)){
			...
			printk(KERN_ERR "[CPUHOTPLUG] %s: ida_pre_get failed in create_worker\n", __func__); 
			goto fail;
		}
		spin_lock_irq(&gcwq->lock);
	}
	...
	if (!worker){
		...
		printk(KERN_ERR "[CPUHOTPLUG] %s: alloc_worker failed.\n", __func__);
		goto fail;
	}
	...
	if (IS_ERR(worker->task)){
		...
		printk(KERN_ERR "[CPUHOTPLUG] %s: kthread_create failed.\n", __func__);
		goto fail;
	}
...
restart:
	...
	while (true) {
		...
		if (worker) {
			...
			spin_lock_irq(&gcwq->lock);
			start_worker(worker);
   			if (WARN_ON_ONCE(need_to_create_worker(gcwq)))
				goto restart;
			return true;
		}
				      ...
				      int flush_color, int work_color)
{
	...
	if (flush_color >= 0) {
		WARN_ON_ONCE(atomic_read(&wq->nr_cwqs_to_flush));
	...
	for_each_cwq_cpu(cpu, wq) {
		...
		if (flush_color >= 0) {
			WARN_ON_ONCE(cwq->flush_color != -1);
		...
		if (work_color >= 0) {
			WARN_ON_ONCE(work_color != work_next_color(cwq->work_color));
...
void destroy_workqueue(struct workqueue_struct *wq)
{
	...
	for_each_cwq_cpu(cpu, wq) {
		...
		for (i = 0; i < WORK_NR_COLORS; i++)
			WARN_ON(cwq->nr_in_flight[i]);
		WARN_ON(cwq->nr_active);
		WARN_ON(!list_empty(&cwq->delayed_works));
	}
						...
						void *hcpu)
{
	...
	case CPU_DOWN_PREPARE:
		...
		if (IS_ERR(new_trustee)){
			...
			printk(KERN_ERR "[CPUHOTPLUG] create new_trustee failed.\n");
	...
	case CPU_UP_PREPARE:
		...
		if (!new_worker) {
			...
			printk(KERN_ERR "[CPUHOTPLUG] create_worker failed.\n");
			if (new_trustee)
...
void freeze_workqueues_begin(void)
{
	...
	for_each_gcwq_cpu(cpu) {
		...
		WARN_ON_ONCE(gcwq->flags & GCWQ_FREEZING);
		gcwq->flags |= GCWQ_FREEZING;
