...
TRACE_EVENT(mmc_request_done,
	...
	TP_PROTO(const char* name, int opcode, unsigned start,  int blocks,  s64 time),

	TP_ARGS(name, opcode, start, blocks, time),
	...
	TP_printk("%s: CMD%d start %u blocks %d, %lldms", __entry->name, __entry->opcode,
