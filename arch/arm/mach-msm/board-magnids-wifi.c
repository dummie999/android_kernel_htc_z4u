/* linux/arch/arm/mach-msm/board-magnids-wifi.c
*/
...
static void *magnids_wifi_mem_prealloc(int section, unsigned long size)
{
...
int __init magnids_init_wifi_mem(void)
{
	int i;
...
static struct resource magnids_wifi_resources[] = {
	[0] = {
		.name		= "bcm4329_wlan_irq",
		.start		= MSM_GPIO_TO_INT(MAGNIDS_GPIO_WIFI_IRQ),
		.end		= MSM_GPIO_TO_INT(MAGNIDS_GPIO_WIFI_IRQ),
#ifdef HW_OOB
...
static struct platform_device magnids_wifi_device = {
				.name           = "bcm4329_wlan",
				.id             = 1,
				.num_resources  = ARRAY_SIZE(magnids_wifi_resources),
				.resource       = magnids_wifi_resources,
				.dev            = {
				.platform_data = &magnids_wifi_control,
				},
};

unsigned char *get_wifi_nvs_ram(void);

static unsigned magnids_wifi_update_nvs(char *str)
{
...
int magnids_wifi_get_mac_addr(unsigned char *buf)
{
	...
	printk(KERN_INFO"magnids_wifi_get_mac_addr = %02x %02x %02x %02x %02x %02x \n",
...
#else
	magnids_wifi_update_nvs("sd_oobonly=1\n");
#endif
	magnids_wifi_update_nvs("btc_params80=0\n");
	magnids_wifi_update_nvs("btc_params6=30\n");
	magnids_init_wifi_mem();
	ret = platform_device_register(&magnids_wifi_device);
	return ret;
}