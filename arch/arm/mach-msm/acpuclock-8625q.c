...
unsigned int current_acpu_rate = 600000;
					...
					enum setrate_reason reason)
{
	...
	if (tgt_s->pll == ACPU_PLL_4) {
		if (strt_s->pll == ACPU_PLL_4 ||
			...
			current_acpu_rate = backup_s->a11clk_khz;
#ifdef CONFIG_HTC_ACPU_DEBUG
			msm_proc_comm(PCOM_BACKUP_CPU_AXI_RATE, (unsigned*)&backup_s->a11clk_khz, (unsigned*)&backup_s->axiclk_khz);
#endif
...
int pll_clk_is_able_close(unsigned pll) {
    unsigned int cpu_rate = current_acpu_rate;

	if ( cpu_rate >= 700800 && pll == 4) goto __error_pll_cpu_rate;
	if ( cpu_rate == 600000 && pll == 2) goto __error_pll_cpu_rate;
	if ( cpu_rate == 480000 && pll == 0) goto __error_pll_cpu_rate;
	if ( cpu_rate == 320000 && pll == 0) goto __error_pll_cpu_rate;
	if ( cpu_rate == 245760 && pll == 1) goto __error_pll_cpu_rate;
	if ( cpu_rate == 19200  && pll == 5) goto __error_pll_cpu_rate;

	return 1;

__error_pll_cpu_rate:
	printk("%s: BUG!!! pll:%u cpu_rate:%u\n", __func__, pll, cpu_rate);
	WARN_ON(1);
	return 0;
}
EXPORT_SYMBOL(pll_clk_is_able_close);
