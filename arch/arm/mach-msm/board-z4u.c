...
static void nfc_gpio_init(void)
{
	...
	GPIO_CFG(Z4U_GPIO_NFC_INT, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	};
...
{
	static uint32_t nfc_gpio_table[] = {
	GPIO_CFG(Z4U_GPIO_NFC_INT, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
	};
	printk(KERN_INFO"[NFC] %s, config NFC_IRQ pin\n",__func__);
	gpio_tlmm_config(nfc_gpio_table[0], GPIO_CFG_ENABLE);
...
#define WITHOUT_NFC_CHIP_ONE 0x00038106
#define WITHOUT_NFC_CHIP_TWO 0x00038107
...
static struct i2c_board_info pn544_i2c_boardinfo[] = {
	{
		...
		.irq = MSM_GPIO_TO_INT(Z4U_GPIO_NFC_INT),
	},
};

static struct bma250_platform_data gsensor_bma250_platform_data_evm = {
	.intr = Z4U_GPIO_GSENSORS_INT,
	.chip_layout = 1,
	.layouts = Z4U_LAYOUTS_EVM,
};

static struct i2c_board_info i2c_bma250_devices_evm[] = {
	{
		I2C_BOARD_INFO(BMA250_I2C_NAME_REMOVE_ECOMPASS, \
				0x32 >> 1),
		.platform_data = &gsensor_bma250_platform_data_evm,
		.irq = MSM_GPIO_TO_INT(Z4U_GPIO_GSENSORS_INT),
	},
};
...
static struct synaptics_i2c_rmi_platform_data syn_ts_3k_data[] = {
	{
		...
		.config = {
			...
			0x13, 0x04, 0x1B, 0x00, 0x10, 0x0A, 0x68, 0x60,
			0x68, 0x60, 0x60, 0x68, 0x48, 0x48, 0x2E, 0x2D,
			...
			0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80,
			0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80,
			0x80, 0x80, 0x80, 0x9A, 0x80, 0x00, 0x0F, 0x00,
			0x02, 0x36, 0x46, 0x08, 0x84, 0x10, 0x14, 0x00,
...
void config_z4u_usb_id_gpios(bool output)
{
	int ret = 0;
	if (output) {
...
void config_z4u_batid_usbid_sel_gpios(bool output)
{
	if (output) {
		gpio_set_value(Z4U_GPIO_BATID_USBID_SEL, 1);
		printk(KERN_INFO "[CABLE]%s BATID_USBID_SEL_GPIO, output high\n",  __func__);
	} else {
		gpio_set_value(Z4U_GPIO_BATID_USBID_SEL, 0);
		printk(KERN_INFO "[CABLE]%s BATID_USBID_SEL_GPIO, output low\n",  __func__);
	}
}
...
static struct cable_detect_platform_data cable_detect_pdata = {
	...
	.config_usb_id_gpios = config_z4u_usb_id_gpios,
	.config_batid_usbid_sel_gpios = config_z4u_batid_usbid_sel_gpios,
	.get_adc_cb             = z4u_get_usbid_adc,
};
...
{
	int rc;
	rc = gpio_request(USB_PHY_ID,	"USB_PHY_ID");
	if (rc < 0) {
		pr_err("failed to request %d GPIO\n", USB_PHY_ID);
		return;
	}
	gpio_direction_output(USB_PHY_ID, 1);
	gpio_set_value_cansleep(USB_PHY_ID, 1);
	gpio_free(USB_PHY_ID);
}
...
static void z4u_poweralg_config_init(struct poweralg_config_type *config)
{
	INT32 batt_id = 0;
	...
	switch(pState) {
		...
		case PSTAT_NORMAL:
			if (temp_01c < 0)
				...
				pState = PSTAT_HIGH_STOP;
			else if (490 < temp_01c) /* also implies t <= 550 */
				pState = PSTAT_LIMITED;
...
static UINT16 m_maxim17050_param_id2_1800mah[] = {
	0x0001,
	0x2801, 0x1080,
	0x1A28, 0x0980,
};
...
static UINT16* m_maxim17050_qrtable_tbl[] = {
	...
	m_maxim17050_param_id2_1800mah,
	m_maxim17050_param_id3_1800mah,
	m_maxim17050_param_id4_1860mah
};

static UINT32 capacity[] = {1800, 1860, 1800, 1800, 1860};
...
static struct battery_parameter z4u_battery_parameter = {
	.fl_25 = NULL,
...
static max17050_platform_data max17050_pdev_data = {
	...
	.func_poweralg_config_init = z4u_poweralg_config_init,
	.func_update_charging_protect_flag = z4u_update_charging_protect_flag,
	.r2_kohm = 0,	/* use get_battery_id, doesn't need this */
	.batt_param = &z4u_battery_parameter,
...
static struct htc_headset_1wire_platform_data htc_headset_1wire_data = {
	.tx_level_shift_en      = Z4U_AUD_UART_OEz,
	.uart_sw                = Z4U_AUD_UART_SEL,
	.remote_press	       = 0,
...
static uint32_t headset_cpu_gpio[] = {
	GPIO_CFG(Z4U_AUD_UART_OEz, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(Z4U_AUD_REMO_PRESz, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(Z4U_AUD_UART_SEL, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(Z4U_AUD_UART_RX, 2, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(Z4U_AUD_UART_TX, 2, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
};

static uint32_t headset_1wire_gpio[] = {
	GPIO_CFG(Z4U_AUD_UART_RX, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(Z4U_AUD_UART_TX, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(Z4U_AUD_UART_RX, 2, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(Z4U_AUD_UART_TX, 2, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
 };
...
static void uart_tx_gpo(int mode)
{
	switch (mode) {
		case 0:
			...
			gpio_set_value_cansleep(Z4U_AUD_UART_TX, 0);
			pr_info("[HS_BOARD]UART TX GPO 0\n");
			break;
		case 1:
			gpio_tlmm_config(headset_1wire_gpio[1], GPIO_CFG_ENABLE);
			gpio_set_value_cansleep(Z4U_AUD_UART_TX, 1);
...
{
	gpio_set_value_cansleep(Z4U_AUD_UART_OEz, enable);
...
static void headset_init(void)
{
	...
	gpio_set_value(Z4U_AUD_UART_OEz, 1); /*Disable 1-wire level shift by default*/
	gpio_set_value(Z4U_AUD_UART_SEL, 0);
...
{
	int insert = 0;
	gpio_set_value(Z4U_AUD_UART_OEz, 1);
...
static void __init msm_z4u_init(void)
{
	...
	if (htc_get_board_revision() == BOARD_EVM) {
		...
		i2c_register_board_info(MSM_GSBI1_QUP_I2C_BUS_ID,
			i2c_bma250_devices_evm,
			ARRAY_SIZE(i2c_bma250_devices_evm));
	} else {
		printk(KERN_INFO "[GSNR]: Use EVT layout\n");
		i2c_register_board_info(MSM_GSBI1_QUP_I2C_BUS_ID,
			i2c_bma250_devices_evt,
