...
static int pll_clk_enable(struct clk *c)
{
	...
	msm_proc_comm(PCOM_READ_CPU_RATE, &cpu_rate, 0);
	msm_proc_comm(PCOM_BACKUP_PLL_CLK_ENABLE_ENTER, 0, 0);
#endif
	...
	if (!pll_control->pll[PLL_BASE + pll_id].on) {
		...
		{
			unsigned pll_on = 1;
			msm_proc_comm(PCOM_BACKUP_PLL_STATUS, (unsigned*) &pll_id, &pll_on);
		}
#endif
	}
...
#ifdef CONFIG_HTC_ACPU_DEBUG
	msm_proc_comm(PCOM_BACKUP_PLL_CLK_ENABLE_LEAVE, 0, 0);
#endif
	return 0;
}
...
static void pll_clk_disable(struct clk *c)
{
...
#ifdef CONFIG_HTC_ACPU_DEBUG
	msm_proc_comm(PCOM_BACKUP_PLL_CLK_DISABLE_ENTER, 0, 0);
#endif
	remote_spin_lock(&pll_lock);
	...
	remote_spin_unlock(&pll_lock);
#ifdef CONFIG_HTC_ACPU_DEBUG
	msm_proc_comm(PCOM_BACKUP_PLL_CLK_DISABLE_LEAVE, 0, 0);
#endif
}
