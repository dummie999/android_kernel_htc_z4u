...
#define CP3DCG_GPIO_NFC_INT     (114)
...
static struct i2c_board_info pn544_i2c_boardinfo[] = {
	{
		...
		.irq = MSM_GPIO_TO_INT(CP3DCG_GPIO_NFC_INT),
	},
};
...
static void cp3dcg_poweralg_config_init(struct poweralg_config_type *config)
{
	INT32 batt_id = 0;
...
static int cp3dcg_update_charging_protect_flag(int ibat_ma, int vbat_mv, int temp_01c, BOOL* chg_allowed, BOOL* hchg_allowed, BOOL* temp_fault)
{
	static int pState = 0;
...
static struct htc_battery_platform_data htc_battery_pdev_data = {
	.gpio_mbat_in = CP3DCG_GPIO_MBAT_IN,
...
static struct battery_parameter cp3dcg_battery_parameter = {
	.fl_25 = NULL,
...
static struct htc_headset_pmic_platform_data htc_headset_pmic_data = {
	...
	.key_gpio		= CP3DCG_AUD_REMO_PRESz,
	.key_irq		= 0,
	.key_enable_gpio	= 0,
	.adc_mic		= 0,
...
static struct htc_headset_1wire_platform_data htc_headset_1wire_data = {
	.tx_level_shift_en      = CP3DCG_AUD_UART_OEz,
	.uart_sw                = CP3DCG_AUD_UART_SEL,
	.remote_press	       = 0,
...
static uint32_t headset_cpu_gpio[] = {
	GPIO_CFG(CP3DCG_AUD_UART_OEz, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(CP3DCG_AUD_REMO_PRESz, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(CP3DCG_AUD_UART_SEL, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(CP3DCG_AUD_UART_RX, 2, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(CP3DCG_AUD_UART_TX, 2, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
};

static uint32_t headset_1wire_gpio[] = {
	GPIO_CFG(CP3DCG_AUD_UART_RX, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(CP3DCG_AUD_UART_TX, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(CP3DCG_AUD_UART_RX, 2, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(CP3DCG_AUD_UART_TX, 2, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
 };
...
static void uart_tx_gpo(int mode)
{
	switch (mode) {
		case 0:
			...
			gpio_set_value_cansleep(CP3DCG_AUD_UART_TX, 0);
			pr_info("[HS_BOARD]UART TX GPO 0\n");
			break;
		case 1:
			gpio_tlmm_config(headset_1wire_gpio[1], GPIO_CFG_ENABLE);
			gpio_set_value_cansleep(CP3DCG_AUD_UART_TX, 1);
...
{
	gpio_set_value_cansleep(CP3DCG_AUD_UART_OEz, enable);
...
static void __init msm_cp3dcg_init(void)
{
	...
	if (htc_get_board_revision() >= BOARD_CPEDCG_EVT_XC) {
		...
		i2c_register_board_info(MSM_GSBI1_QUP_I2C_BUS_ID,
			i2c_bma250_devices_xc, ARRAY_SIZE(i2c_bma250_devices_xc));
	} else if (htc_get_board_revision()) {
...
static void __init cp3dcg_fixup(struct tag *tags, char **cmdline, struct meminfo *mi)
{
